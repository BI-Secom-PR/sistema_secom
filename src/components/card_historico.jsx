...
function CardsHistorico({ startDate, endDate, selectedCampaign }) {
  ...
  const colors = {
    ...
    secondary: isDarkMode ? '#94a3b8' : '#1E293B',
    border: isDarkMode ? '#585858' : '#E2E8F0',
    background: isDarkMode ? '#2c2c2c' : '#F8FAFC',
    lightBg: isDarkMode ? '#3f3f3f' : '#F1F5F9',
    cardBg: isDarkMode 
      ? 'linear-gradient(145deg, #353535, #272727)'
    ...
    comparison: {
      positive: '#10B981', // Verde mantido em ambos os temas
      negative: '#EF4444'  // Vermelho mantido em ambos os temas
    }
  };
  ...
  const styles = {
    ...
    campaignValue: {
      ...
      fontWeight: '600',
      color: isDarkMode ? '#90cdf4' : '#224dda', // Blue adjusted for dark mode
      display: 'flex',
      alignItems: 'center',
    ...
    spinnerColor: isDarkMode ? "light" : "primary" // Adjust spinner color for dark mode
  };
  ...
  const getIcon = (type) => {
    ...
    const iconContainerStyle = {
      ...
      top: '-20px',
      right: '-20px',
      opacity: isDarkMode ? 0.15 : 0.2, // Slightly lower opacity in dark mode
      zIndex: 1,
    };
      ...
      case 'CPM':
        return (
          <div style={iconContainerStyle}>
            <img src={`https://img.icons8.com/?size=100&id=aNNKpmr4wFqy&format=png&color=${iconColor}`} alt="CPM" width={iconSize} height={iconSize} />
          </div>
        );
      case 'CPV':
        return (
          <div style={iconContainerStyle}>
            <img src={`https://img.icons8.com/?size=100&id=cNMeoarsRjIB&format=png&color=${iconColor}`} alt="CPV" width={iconSize} height={iconSize} />
          </div>
        );
      case 'CPC':
        return (
          <div style={iconContainerStyle}>
            <img src={`https://img.icons8.com/?size=100&id=rt10HSNWQOvx&format=png&color=${iconColor}`} alt="CPC" width={iconSize} height={iconSize} />
          </div>
        );
      case 'CTR':
        return (
          <div style={iconContainerStyle}>
            <img src={`https://img.icons8.com/?size=100&id=WWgXYpYTltbk&format=png&color=${iconColor}`} alt="CTR" width={iconSize} height={iconSize} />
          </div>
        );
      case 'VTR':
        return (
          <div style={iconContainerStyle}>
            <img src={`https://img.icons8.com/?size=100&id=aLWI1DFiVtY2&format=png&color=${iconColor}`} alt="VTR" width={iconSize} height={iconSize} />
          </div>
        );
      default:
        return null;
      ...
      <div 
        style={{
          ...
          backgroundColor: comparison.isPositive 
            ? `${colors.comparison.positive}${isDarkMode ? '20' : '15'}` // Slightly darker bg in dark mode
            : `${colors.comparison.negative}${isDarkMode ? '20' : '15'}`,
              ...
              <Card.Body style={styles.cardBody}>
                ...
                {loading ? (
                  <div style={styles.loadingContainer}>
                    <Spinner animation="border" variant={styles.spinnerColor} />
                  </div>
                ) : (
