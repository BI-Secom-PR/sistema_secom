  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      if (response.data && response.data.token) {
        ...
        storage.setItem('_role', response.data.user._role);
      ...
      <Col md={6} className="d-none d-md-flex flex-column p-4" style={{ backgroundColor: 'white' }}>
        <div className="d-flex flex-column justify-content-center h-100">
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
            <Image src={dashboardImg} alt="Dashboard" style={{ width: 'auto', maxWidth: '100%', height: 'auto', marginLeft: '-16rem' }} />
          </div>
        </div>
      </Col>

      <Col md={6} className="p-4" style={{ backgroundColor: '#f8f9fa' }}>
        <Image src={govLogo} alt="Governo Federal" style={{ width: '280px', marginBottom: '3rem' }} className="d-block mx-auto" />
